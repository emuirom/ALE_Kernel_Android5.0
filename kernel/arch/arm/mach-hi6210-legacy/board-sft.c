...
static void sram_reboot()
{
     ...
     *(volatile int *)SOC_MDDRC_DMC_DDRC_CTRL_SREF_ADDR(SOC_MDDRC_DMC_BASE_ADDR) = 0x1;
    while((*(volatile int *)SOC_MDDRC_DMC_DDRC_CURR_FUNC_ADDR(SOC_MDDRC_DMC_BASE_ADDR)) & 0x1 == 0)
    {
        *(volatile int *)SOC_MDDRC_DMC_DDRC_CURR_FUNC_ADDR(SOC_MDDRC_DMC_BASE_ADDR) == 0x1;
    }
    while (1) {
...
#warning "add some sys ctrl register operation here, such as enable something .etc"
	/*cache_init();*/  /*l2 cache exist in a7 inside*/
...
MACHINE_START(HI6210SFT, "hi6210sft")
