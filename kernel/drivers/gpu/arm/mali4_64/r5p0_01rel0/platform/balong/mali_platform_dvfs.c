...
static mali_dvfs_threshold_table mali_dvfs_threshold[3]={
    /*step by step profile 0*/{((int)((255*40)/100)), ((int)((255*80)/100))},
    /*step by step profile 1*/{((int)((255*40)/100)), ((int)((255*90)/100))},
    /*step by step profile 2*/{((int)((255*65)/100)), ((int)((255*90)/100))}
};

static mali_dvfs_threshold_table mali_dvfs_over_threshold={
    /*step over*/   ((int)((255*20)/100)), ((int)((255*95)/100))
};
...
mali_dvfs_status decideNextStatus(void)
{
    ...
    /*get the decision*/
    if (utilization > mali_dvfs_over_threshold.upthreshold)
    {
        dvfsStatus      = MALI_DVFS_INCREASE_HIGHEST;
    }
    else if ( (utilization <= mali_dvfs_over_threshold.upthreshold)
            &&(utilization > mali_dvfs_threshold[g_mali_dfs_var.dfs_CurrPrf].upthreshold))
    {
        dvfsStatus      = MALI_DVFS_INCREASE;
    }
    else if ( (utilization <= mali_dvfs_threshold[g_mali_dfs_var.dfs_CurrPrf].downthreshold)
            &&(utilization > mali_dvfs_over_threshold.downthreshold))
    {
        dvfsStatus      = MALI_DVFS_DECREASE;
    }
    else if ( (utilization <= mali_dvfs_over_threshold.downthreshold))
    {
...
void mali_dvfs_status_update(u32 utilization)
{
    ...
    MALI_DEBUG_PRINT(4, ("mali_dvfs_status: utilization = %d with count %d \n",utilization,g_mali_dfs_var.dfs_StayCount));
    ...
    MALI_DEBUG_PRINT(4, ("nextStatus %d, currentStep %d \n", nextStatus, g_stMaliDvfsStatus.currentStep));
...
{
    MALI_DEBUG_PRINT(4, ("=== mali_dvfs_work_handler\n"));
...
static int gpu_devfreq_probe(struct platform_device *pdev)
{
	...
	if (of_init_opp_table(&pdev->dev) ||
	    ...
	    gpu_devfreq_dev_profile.initial_freq = g_mali_dvfs_profile[0].freq * GPU_MHZ;
		rcu_read_lock();
		gpu_devfreq_dev_profile.max_state = opp_get_opp_count(&pdev->dev);
		rcu_read_unlock();
