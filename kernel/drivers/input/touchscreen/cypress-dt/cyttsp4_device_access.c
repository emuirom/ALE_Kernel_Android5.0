		...
		size_t length, int num_read, int offset, int mode)
{
	...
	if (length < num_read) {
		dev_err(dev, "%s: not sufficient buffer req_bug_len=%d, length=%lu\n",
		...
		size_t length)
{
	...
	if (length < num_read) {
		dev_err(dev,
			"%s: not sufficient buffer req_bug_len=%d, length=%lu\n",
			__func__, num_read, length);
		return -EINVAL;
	}
		...
		size_t length)
{
	...
	if (length < return_buf_size) {
		dev_err(dev, "%s: not sufficient buffer req_buf_len=%d, length=%lu\n",
		...
		u8 *ic_buf, size_t length)
{
	...
	if (length < num_read) {
		dev_err(dev, "%s: not sufficient buffer %s=%d, %s=%lu\n",
		...
		size_t length)
{
	...
	if (dad->test.cur_cmd == CY_CMD_CAT_NULL) {
		...
		dev_vdbg(dev, "%s: GRP=TEST_REGS: num_rd=%d at ofs=%lu + grpofs=%u\n",
		...
		size_t length)
{
	...
	if (dad->test.cur_cmd == CY_CMD_CAT_NULL) {
		...
		case CY_NULL_CMD_STATUS_SIZE:
			...
			dev_vdbg(dev, "%s: test-cur_status_size=%lu\n",
		...
		dev_dbg(dev, "%s: TEST CMD=0x%02X length=%lu %s%lu\n",
		...
		size_t buf_size, u8 *ic_buf, size_t ic_buf_size)
{
	...
	dev_dbg(dev, "%s: pbuf=%p buf=%p size=%lu %s=%lu buf=%s\n", __func__,
	...
	while (pbuf <= (buf + buf_size)) {
		...
		if (i >= ic_buf_size) {
			dev_err(dev, "%s: %s size=%d buf_size=%lu\n", __func__,
