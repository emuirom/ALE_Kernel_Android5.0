...
#include <linux/app_info.h> //h00216361
		...
		const u8 *fw_img, int fw_size)
{
	...
	else
	{
		if (data->loader_pdata &&
			(data->loader_pdata->flags & CY_LOADER_FLAG_CALIBRATE_AFTER_FW_UPGRADE))
		{
...
static int upgrade_firmware_from_builtin(struct cyttsp4_device *ttsp)
{
	...
	//dev_vdbg(dev, "%s: firmware name is:%s\n", __func__,CY_FW_FILE_NAME);//huawei 0701
	dev_vdbg(dev, "%s: firmware name is:%s\n", __func__,fw_name);//huawei 0701
...
static int upgrade_ttconfig_from_platform(struct cyttsp4_device *ttsp)
{
	...
	for (ttconfig_map = data->loader_pdata->ttconfig_map;
		ttconfig_map->ttconfig_param;
		ttconfig_map++)
	{
		dev_info(dev, "%s: panel_id=%d,  ttconfig_map->id=%d\n", __func__, panel_id,  ttconfig_map->id);
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_PLATFORM_TTCONFIG_UPGRADE
static int upgrade_ttconfig_from_platform_recovery(struct cyttsp4_device *ttsp)
{
	struct device *dev = &ttsp->dev;
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
	...
	return cyttsp4_upgrade_ttconfig(ttsp, param_regs->data,param_regs->size);
		...
		char *buf, loff_t offset, size_t count)
{
	...
	dev_vdbg(dev, "%s: offset:%lld count:%lu\n", __func__, offset, count);
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	if (6 == pannel_id)
	{
	    sprintf(touch_info,"cypress_TMA463_OFILM.%d",ttconfig_version);

	}
	else if (7 == pannel_id)
	{
	    sprintf(touch_info,"cypress_TMA463_TRULY.%d",ttconfig_version);

	}
	rc = app_info_set("touch_panel", touch_info);
	if (rc)
	{
		dev_err(dev, "set_touch_chip_info error\n");
	}
	return;
}
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_BINARY_FW_UPGRADE
	dev_attr_manual_upgrade.attr.mode = 0;
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_MANUAL_TTCONFIG_UPGRADE
	dev_attr_config_loading.attr.mode = 0;
