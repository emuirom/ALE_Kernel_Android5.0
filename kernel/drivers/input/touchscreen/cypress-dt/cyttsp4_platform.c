...
/* factory version use specific config */
#ifndef CONFIG_FACTORY_MODE_FEATURE
/*change the name of the config file*/
#include <linux/Config_Ofilm.h>
static struct touch_settings cyttsp4_sett_ofilm_param_regs = {
	.data = (uint8_t *)&cyttsp4_ofilm_param_regs[0],
	.size = ARRAY_SIZE(cyttsp4_ofilm_param_regs),
	.tag = 0,
};
static struct touch_settings cyttsp4_sett_ofilm_param_size = {
	.data = (uint8_t *)&cyttsp4_ofilm_param_size[0],
	.size = ARRAY_SIZE(cyttsp4_ofilm_param_size),
	.tag = 0,
};

#include <linux/Config_Truly.h>
static struct touch_settings cyttsp4_sett_truly_param_regs = {
	.data = (uint8_t *)&cyttsp4_truly_param_regs[0],
	.size = ARRAY_SIZE(cyttsp4_truly_param_regs),
	.tag = 0,
};
static struct touch_settings cyttsp4_sett_truly_param_size = {
	.data = (uint8_t *)&cyttsp4_truly_param_size[0],
	.size = ARRAY_SIZE(cyttsp4_truly_param_size),
	.tag = 0,
};
#else
#include <linux/Config_Ofilm.h>
static struct touch_settings cyttsp4_sett_ofilm_param_regs = {
	.data = (uint8_t *)&cyttsp4_ofilm_param_ftm_regs[0],
	.size = ARRAY_SIZE(cyttsp4_ofilm_param_ftm_regs),
	.tag = 0,
};
static struct touch_settings cyttsp4_sett_ofilm_param_size = {
	.data = (uint8_t *)&cyttsp4_ofilm_param_ftm_size[0],
	.size = ARRAY_SIZE(cyttsp4_ofilm_param_ftm_size),
	.tag = 0,
};

#include <linux/Config_Truly.h>
static struct touch_settings cyttsp4_sett_truly_param_regs = {
	.data = (uint8_t *)&cyttsp4_truly_param_ftm_regs[0],
	.size = ARRAY_SIZE(cyttsp4_truly_param_ftm_regs),
	.tag = 0,
};
static struct touch_settings cyttsp4_sett_truly_param_size = {
	.data = (uint8_t *)&cyttsp4_truly_param_ftm_size[0],
	.size = ARRAY_SIZE(cyttsp4_truly_param_ftm_size),
	.tag = 0,
};
#endif

static struct cyttsp4_touch_config cyttsp4_ofilm_ttconfig = {
	.param_regs = &cyttsp4_sett_ofilm_param_regs,
	.param_size = &cyttsp4_sett_ofilm_param_size,
	.fw_ver = ofilm_ttconfig_fw_ver,
	.fw_vsize = ARRAY_SIZE(ofilm_ttconfig_fw_ver),
};
static struct cyttsp4_touch_config cyttsp4_truly_ttconfig = {
	.param_regs = &cyttsp4_sett_truly_param_regs,
	.param_size = &cyttsp4_sett_truly_param_size,
	.fw_ver = truly_ttconfig_fw_ver,
	.fw_vsize = ARRAY_SIZE(truly_ttconfig_fw_ver),
};
...
struct cyttsp4_sett_ttconfig_map cyttsp4_sett_ttconfig_map[] = {
		[0]={
				.id=6,
				.ttconfig_param = &cyttsp4_ofilm_ttconfig,
			},
		[1]={
				.id=7,
				.ttconfig_param = &cyttsp4_truly_ttconfig,
			},
		[2]={
				.ttconfig_param = NULL,
			},
		...
		int on, struct device *dev)
{
	...
	if (on)
    {
		rc = gpio_request(rst_gpio, "cyttsp4_rst");
		if (rc < 0)
        {
			gpio_free(rst_gpio);
			rc = gpio_request(rst_gpio, "cyttsp4_rst");
        ...
        else
        {
		    ...
		    //rc = gpio_tlmm_config(GPIO_CFG(irq_gpio,0,GPIO_CFG_INPUT,
                ...
                //if( rc < 0 )
                //{
                //    dev_err(dev,"%s: Fail set gpio=%d as no pull\n", __func__, irq_gpio);
                //}
