...
static void  hw_isr_do_tasklet(unsigned long data)
{
    ...
    /* PIPELINE0 VSYNC*/
    if(pipe0_status & ISP_PIPE0_VSYNC_IDI) {
        ovisp23_notify_vsync(0, tm_v[0].tv_sec * 1000000L + tm_v[0].tv_usec);
    }

    /* PIPELINE1 VSYNC*/
    if(pipe0_status & ISP_PIPE1_VSYNC_IDI) {
        ovisp23_notify_vsync(1, tm_v[0].tv_sec * 1000000L + tm_v[0].tv_usec);
    }
}
