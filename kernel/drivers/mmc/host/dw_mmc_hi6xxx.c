...
static int dw_mci_hi6xxx_slowdown_clk(struct dw_mci *host, int timing)
{
	struct dw_mci_hs_priv_data *priv = host->priv;
	int id = priv->id;
	...
	else
	{
			...
			*/
			...
			if (host->vqmmc)
			{
				...
				if (ret)
				{
					dev_warn(host->dev, "Switching to 3.0V signalling "
							"voltage failed\n");
					return -EIO; }
...
static void dw_mci_hi6xxx_tuning_fail_excprocess(struct dw_mci *host)
{
    ...
    /*配置SC_PERIPH_RSTEN0[0]:使能mmc0 ip复位*/
        writel(0x1<<SOC_PERI_SCTRL_SC_PERIPH_RSTEN0_periph_rsten0_mmc0_START,
		   SOC_PERI_SCTRL_SC_PERIPH_RSTEN0_ADDR(priv->peri_sysctrl));

	udelay(10);

    /*配置SC_PERIPH_RSTDIS0[0]:去使能mmc0 ip复位*/
        writel(0x1<<SOC_PERI_SCTRL_SC_PERIPH_RSTDIS0_periph_rstdis0_mmc0_START,
		   SOC_PERI_SCTRL_SC_PERIPH_RSTDIS0_ADDR(priv->peri_sysctrl));
#endif
    /*mmc0 ip复位后检查寄存器*/
	...
	printk(KERN_ERR "before  mshci_tuning_fail_excprocess: CTRL=%x, INTMASK=%x,"
	...
	dw_mci_hi6xxx_clear_set_irqs(host, INTMSK_ALL,
		INTMSK_CDETECT | INTMSK_RE |
		INTMSK_CDONE | INTMSK_DTO |
		INTMSK_RCRC | INTMSK_DCRC | INTMSK_RTO | INTMSK_DRTO |
		INTMSK_HTO | INTMSK_HLE | INTMSK_SBE |
		INTMSK_EBE);
...
tryagain:
        ...
        if (cmd.error != 0 || data.error != 0 || memcmp(data_buf, tuning_block_128, sizeof(tuning_block_128))!= 0)
        {
            ...
            else
            {
                ...
                /*SEL SYS-PLL*/
                writel(readl(SOC_PERI_SCTRL_SC_CLK_SEL0_ADDR(priv->peri_sysctrl))|(0x1<<SOC_PERI_SCTRL_SC_CLK_SEL0_sel_clk_mmc_START),
                       SOC_PERI_SCTRL_SC_CLK_SEL0_ADDR(priv->peri_sysctrl));
...
int dw_mci_hi6xxx_sd_tuning(struct dw_mci_slot *slot)
{
    ...
    writel(readl(SOC_PERI_SCTRL_SC_PERIPH_CTRL2_ADDR(priv->peri_sysctrl))&(~(0x1<<SOC_PERI_SCTRL_SC_PERIPH_CTRL2_mmc_clk_phase_bypass_en_mmc1_START) ),
           SOC_PERI_SCTRL_SC_PERIPH_CTRL2_ADDR(priv->peri_sysctrl));
...
tryagain:
    ...
    else
    {
        ...
        /*SOC1002:bypass en，低频*/
        writel(readl(SOC_PERI_SCTRL_SC_PERIPH_CTRL2_ADDR(priv->peri_sysctrl))|(0x4),
               SOC_PERI_SCTRL_SC_PERIPH_CTRL2_ADDR(priv->peri_sysctrl));
...
static const struct dw_mci_drv_data hi6xxx_drv_data = {
	...
	.tuning_find_condition
						= dw_mci_hi6xxx_tuning_find_condition,
	.tuning_set_current_state
						= dw_mci_hi6xxx_tuning_set_current_state,
