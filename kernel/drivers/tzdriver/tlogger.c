...
start:
    ...
    if(disjoin_flag && ((reader->r_off + log_len )>  addr_end)){
        if(CHECK_BOUNDARY(reader->r_off) ){
            TLOGERR("1 CHECK_BOUNDARY error reader->r_off=0x%llu \n", reader->r_off);
        }
        ...
        if(CHECK_BOUNDARY(reader->r_off) || CHECK_BOUNDARY(reader->r_off+log_len)){
            TLOGERR("2 CHECK_BOUNDARY error reader->r_off=0x%llu reader->r_off+log_len =0x%llu\n",reader->r_off, reader->r_off+log_len);
        }
...
#define ADDR_P (0x3fe00000+5*1024) //log start physical address
#define OFFSET1 64
...
//#define RDR_LOG_SIZE (20*1024) //for test
#define RDR_LOG_SIZE (480*1024)

static int __init tlogger_init(void)
{
    int ret;
    log_buffer = (char *)ioremap_wc(ADDR_P,500*1024);
    addr_offset =(size_t)(log_buffer -(char *) ADDR_P );
    addr_start = log_buffer + 64;
    addr_end =  log_buffer + RDR_LOG_SIZE;
    TLOGEBUG("logcat log_buffer=%llx  addr_offset=%llx\n",log_buffer,addr_offset);
    TLOGEBUG("logcat addr_start=%llx  addr_end=%llx\n",addr_start,addr_end);

    ret = create_log(LOGGER_LOG_TEEOS, 480*1024);
    return ret;
}
