...
static struct usb_endpoint_descriptor gser_ss_notify_desc  = {
    ...
    .bmAttributes =        USB_ENDPOINT_XFER_INT,
    .wMaxPacketSize =    __constant_cpu_to_le16(GS_NOTIFY_MAXPACKET),
    .bInterval =        GS_LOG2_NOTIFY_INTERVAL+8,
};
#endif
...
static int gser_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
    ...
    rc = config_ep_by_speed(cdev->gadget, f, gser->notify);
	if (rc) {
        ERROR(cdev, "can't config_ep_by_speed notify %s, result %d\n",
                    gser->notify->name, rc);
        return rc;
    }
    ...
    rc = config_ep_by_speed(cdev->gadget, f, gser->port.in);
	if (rc) {
        ERROR(cdev, "can't config_ep_by_speed in %s, result %d\n",
                    gser->notify->name, rc);
        return rc;
    }

    rc = config_ep_by_speed(cdev->gadget, f, gser->port.out);
	if (rc) {
        ERROR(cdev, "can't config_ep_by_speed out %s, result %d\n",
                    gser->notify->name, rc);
        return rc;
    }
...
#ifdef CONFIG_MODEM_SUPPORT
    printk(KERN_INFO "modem flush fifo\n");
...
int gser_send_carrier_detect(struct gserial *port, unsigned int yes)
{
    struct f_gser *gser = port_to_gser(port);
    u16            state;
...
int gser_send_ring_indicator(struct gserial *port, unsigned int yes)
{
    struct f_gser *gser = port_to_gser(port);
    u16            state;
    state = gser->serial_state;
 ...
 */
int modem_bind_config(struct usb_configuration *c, u8 port_num)
{
    struct f_gser    *gser;
    int        status;
