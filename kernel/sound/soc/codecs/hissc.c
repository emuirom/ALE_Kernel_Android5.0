...
static void hissc_dapm_widget_dp(char *buf)
{
	...
	list_for_each_entry(w, l, list) {
		i++;
		snprintf(buf, HISSC_DBG_SIZE_WIDGET, "%s<%02d> pwr= %d \"%s\"\n", buf, i, w->power, w->name);
	}
	snprintf(buf, HISSC_DBG_SIZE_WIDGET, "%sEND\n", buf);
}
...
static void hissc_dapm_path_dp(char *buf)
{
	...
	list_for_each_entry(p, l, list) {
		i++;
		snprintf(buf, HISSC_DBG_SIZE_PATH, "%s<%d>cn_stat: %d\n", buf, i, p->connect);
		if(p->source == NULL || p->sink == NULL) {
			snprintf(buf, HISSC_DBG_SIZE_PATH, "%s<%d>end\n", buf, i);
			continue;
		}
		...
		snprintf(buf, HISSC_DBG_SIZE_PATH, "%s src w \"%s\"\n", buf, source->name);
		snprintf(buf, HISSC_DBG_SIZE_PATH, "%ssink w \"%s\"\n", buf, sink->name);
	}
	snprintf(buf, HISSC_DBG_SIZE_PATH, "%sEND\n", buf);
}
...
static void hissc_reg_history_dp(char *buf)
{
	...
	for (idx = idx_start; idx < idx_stop; idx++) {
		...
		if (REG_CACHE_FLAG_READ == (reg & REG_CACHE_FLAG_MASK)) {
			/* read */
			snprintf(buf, HISSC_DBG_SIZE_CACHE, "%s%d.r %04X:%02X\n", buf, time, reg & ~REG_CACHE_FLAG_MASK, val);
		} else if (REG_CACHE_FLAG_WRITE == (reg & REG_CACHE_FLAG_MASK)) {
			/* write */
			snprintf(buf, HISSC_DBG_SIZE_CACHE, "%s%d.w %04X %02X\n", buf, time, reg & ~REG_CACHE_FLAG_MASK, val);
		} else {
			/* error branch */
			snprintf(buf, HISSC_DBG_SIZE_CACHE, "%s%d.e %x %x\n", buf, time, reg & ~REG_CACHE_FLAG_MASK, val);
		}
		spin_unlock_irqrestore(&priv->lock, flags);
	}
	...
	spin_unlock_irqrestore(&priv->lock, flags);
	snprintf(buf, HISSC_DBG_SIZE_CACHE, "%sidx=%d -- %d, END\n", buf, idx_wr_now, idx_wr_latter);
}
...
static void hissc_page_dp(char *buf)
{
	...
	for  (regi = HISSC_PAGE_0_START; regi <= HISSC_PAGE_0_END; regi += 4) {
		snprintf(buf, HISSC_DBG_SIZE_PAGE, "%s%#04x:%#010x\n", buf, regi, hissc_reg_read(g_codec, regi));
	}
	
	for (regi = HISSC_PAGE_0C_START; regi <= HISSC_PAGE_0C_END; regi += 4) {
		snprintf(buf, HISSC_DBG_SIZE_PAGE, "%s%#04x:%#010x\n", buf, regi, hissc_reg_read(g_codec, regi));
	}
	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%s\n", buf);

	/* page 1 */
	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%sPage 1\n", buf);
	for (regi = HISSC_PAGE_1_START; regi <= HISSC_PAGE_1_END; regi++) {
		snprintf(buf, HISSC_DBG_SIZE_PAGE, "%s%#04x:%#04x\n", buf, regi, hissc_reg_read(g_codec, regi));
	}
	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%s\n", buf);

	/* page 2 */
	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%sPage 2\n", buf);
	...
	/* page 4 - pmu */
	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%sPage 4\n", buf);
	...
	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%s<cat end>\n", buf);
	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%s\n", buf);
}
...
static void hissc_state_dp(char *buf)
{
	...
	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%shs_status=%d(old=%d), jack->report=0x%x\n",
		buf, priv->hs_status, priv->old_hs_status, priv->hs_jack->report);

	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%sIRQ=0x%x, IRQM=0x%x, STAT=0x%x\n",
		buf, hissc_reg_read(g_codec, HISSC_SMT_ANA_IRQ_REG0_ADDR),
		hissc_reg_read(g_codec, HISSC_SMT_ANA_IRQM_REG0_ADDR),
		hissc_reg_read(g_codec, HISSC_SMT_ANA_IRQ_SIG_STAT_ADDR));

	snprintf(buf, HISSC_DBG_SIZE_PAGE, "%s<end>\n", buf);
}
				...
				size_t count, loff_t *ppos)
{
	...
	if (INVALID_REG != vs_reg_r) {
		...
		if(PAGE_SoCCODEC == (vs_reg_r & PAGE_TYPE_MASK)) {
			snprintf(kn_buf, HISSC_DBG_SIZE_PAGE, "%s%#04x:%#010x\n", kn_buf, vs_reg_r, hissc_reg_read(g_codec, vs_reg_r));
			snprintf(kn_buf, HISSC_DBG_SIZE_PAGE, "%s<cat end>\n", kn_buf);
		} else {
			snprintf(kn_buf, HISSC_DBG_SIZE_PAGE, "%s%#04x:%#04x\n", kn_buf, vs_reg_r, hissc_reg_read(g_codec, vs_reg_r));
			snprintf(kn_buf, HISSC_DBG_SIZE_PAGE, "%s<cat end>\n", kn_buf);
		}
	} else {
		/* PAGE */
				...
				size_t count, loff_t *ppos)
{
	...
	for (idx_ps = 0;idx_ps < ps_index;idx_ps++) {
		switch(ps_cache[idx_ps].op) {
		case HISSC_POP_DELAY_MS:
			snprintf(kn_buf, HISSC_DBG_SIZE_CACHE, "%sm %d\n",kn_buf,ps_cache[idx_ps].val);
			break;
		case HISSC_POP_DELAY_US:
			snprintf(kn_buf, HISSC_DBG_SIZE_CACHE, "%su %d\n",kn_buf,ps_cache[idx_ps].val);
			break;
		case HISSC_POP_REG_WRITE:
			snprintf(kn_buf, HISSC_DBG_SIZE_CACHE, "%sw 0x%x 0x%x\n",kn_buf,ps_cache[idx_ps].reg,ps_cache[idx_ps].val);
			break;
		default:
			loge("op not found\n");
		}
	}
	snprintf(kn_buf, HISSC_DBG_SIZE_CACHE, "%sPS DUMP END\n",kn_buf);
	byte_read = simple_read_from_buffer(user_buf, count, ppos, kn_buf, strlen(kn_buf));
